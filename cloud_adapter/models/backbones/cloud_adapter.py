...
from .frequency_routed_adapter import FrequencyDecomposer, FrequencyRoutedInteractiveModule
...
class NoAdaptingModule(nn.Identity):
    ...
    def forward(self, x, cache, layer, spatial_shape=None):
        return x
...
class CloudAdapter(nn.Module):
    def __init__(self,
        ...
        if int_type == "convnext":
        ...
        elif int_type == "frequency_routed":
            self.frequency_decomposer = FrequencyDecomposer(cutoff_ratio=cutoff_ratio)
            ...
            self.net = nn.ModuleList(
                FrequencyRoutedInteractiveModule(
                    ...
                    dim_head=frequency_dim_head,
                )
                for _ in range(num_layers)
            )
        else:
            raise ValueError(
                "int_type must be one of "
                f"['convnext', 'pmaa', 'frequency_routed', 'no_adapting'], got {int_type}")

        self.register_buffer(
            "balance_loss_weight",
            torch.tensor(float(lambda_balance)),
            persistent=True,
        )
    ...
    def forward(
        ...
        # 24 // 1
        # feat: 1024 2 1024
        cache_index = min(3, layer * 4 // max(1, len(self.net)))
        if isinstance(self.net[layer], FrequencyRoutedInteractiveModule):
            if layer == 0:
                # A context is reused by six DINOv2-L layers.  Decompose each
                # selected scale once per backbone forward instead of per layer.
                self._frequency_cache = {}
            context = self.net[layer]._select_context(cache, cache_index)
            cache_key = (id(context), tuple(context.shape), context.device)
            if cache_key not in self._frequency_cache:
                self._frequency_cache[cache_key] = self.frequency_decomposer(context)
            feats = self.net[layer](
                feats,
                cache,
                cache_index,
                spatial_shape=spatial_shape,
                precomputed_branches=self._frequency_cache[cache_key],
            )
        else:
            feats = self.net[layer](feats, cache, cache_index)
    ...
    def get_balance_loss(self):
        """Return the mean balance loss from modules used in the last forward."""
        losses = [
            module.last_balance_loss
            for module in self.net
            if isinstance(module, FrequencyRoutedInteractiveModule)
            and module.last_balance_loss is not None
        ]
        if not losses:
            return None
        return torch.stack(losses).mean() * self.balance_loss_weight

    def get_router_statistics(self):
        return [
            module.last_router_stats
            for module in self.net
            if isinstance(module, FrequencyRoutedInteractiveModule)
            and module.last_router_stats
        ]
